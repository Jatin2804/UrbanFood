...
import { clearCart, fetchCart, removeFromCart } from "@/src/features/cart/cartThunks";
...
const CartRow = ({
  ...
  return (
    <ThemedView variant="surface" style={styles.row}>
      {/* Image */}
      <TouchableOpacity onPress={onPress} activeOpacity={0.9} style={styles.imageWrap}>
        <Image
      ...
      {/* Info */}
      <TouchableOpacity onPress={onPress} activeOpacity={0.9} style={styles.rowInfo}>
        <ThemedText style={styles.rowName} numberOfLines={2}>{dish.name}</ThemedText>
        <ThemedText type="small" style={{ color: theme.textTertiary, marginTop: 2 }}>
          ₹{dish.price} × {dish.quantity}
        </ThemedText>
        <ThemedText style={styles.rowTotal}>₹{dish.price * dish.quantity}</ThemedText>
      </TouchableOpacity>
    ...
    <ThemedView style={styles.container}>
      ...
      {dishes.length === 0 ? (
        <View style={styles.emptyContainer}>
          ...
          <ThemedText type="subtitle" style={{ marginTop: Spacing.lg, marginBottom: 6 }}>
            Your cart is empty
          </ThemedText>
...
const styles = StyleSheet.create({
  ...
  rowTotal: { fontSize: 15, fontWeight: "700", color: Brand.primary, marginTop: 4 },
